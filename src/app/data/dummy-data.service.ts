...
export class DummyDataService {
  ...
  getProjects(): Observable<Project[]> {
    return of(null).pipe(
      ...
      switchMap(() => {
        ...
        const mapped = this.DummyProjects.map((project) => ({
          ...project,
  ...
  searchFruits(term: string, page: number, pageSize: number): Observable<SearchResult> {
    ...
    const filtered = this.fruits.filter((fruit) => fruit.label.toLowerCase().includes(lowerTerm));

    const start = (page - 1) * pageSize;
    const paginated = filtered.slice(start, start + pageSize);

    return of({ items: paginated, totalCount: filtered.length }).pipe(delay(500));
  }
